...
public class IntakeSubsystem extends SubsystemBase {
  ...
  public Angle getPivotAngle() {
    if (Constants.currentMode == Constants.Mode.SIM) return Radians.of(pivotSim.getAngleRads());
  ...
  public Trigger intakeHasCoralTrigger() {
    if (Constants.currentMode == Constants.Mode.SIM)
      return new Trigger(() -> intakeSim.getGamePiecesAmount() > 0);
    return new Trigger(() -> intakeHasCoral());
  }
  ...
  public Command floorIntake() {
    return runEnd(
            () -> {
              ...
              if (Constants.currentMode == Constants.Mode.SIM) intakeSim.startIntake();
            },
            () -> {
  ...
  public Command Idle(Supplier<Boolean> elevatorNotL1) {
    return run(() -> {
          ...
          setIntakeMotor(0);
          setConveyerMotor(0);
          if (Constants.currentMode == Constants.Mode.SIM) intakeSim.stopIntake();
        })
        .withName("IDLE");
  }
